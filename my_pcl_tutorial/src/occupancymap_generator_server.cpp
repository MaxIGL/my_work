...
//Reorder the list to match occupancy grid convension
std::vector<int8_t> occupancygridlist_2;
for (unsigned int i=1;i!=height+1;i++){
for (unsigned int j=0;j!=width;j++){
occupancygridlist_2.push_back(occupancygridlist[(height-i)*width+j]);
}
}
...
occupancygrid.data=occupancygridlist_2;
...
if (occupancygridlist_2[j+width*i]==100) mapdiscret[y+x*rectangle_large][1]+=1;
if(occupancygridlist_2[j+width*i]==-1) mapdiscret[y+x*rectangle_large][2]+=1;
}}
