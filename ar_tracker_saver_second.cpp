...
void ar_saver_cb(const ar_track_alvar_msgs::AlvarMarkersConstPtr& ar_tracker,std::string filename)
{
...
int main(int argc, char** argv)
{
  ...
  std::string topic_in_n;
  nh.getParam("topic_in",topic_in_n);
  std::string filename;
  nh.getParam("filename",filename);

  // Create a ROS subscriber for the input point cloud
  ros::Subscriber sub = nh.subscribe<ar_track_alvar_msgs::AlvarMarkers>(topic_in_n, 10, boost::bind(&ar_saver_cb,_1,filename));
